...
Interpolate = _create_enum(
    'Interpolate', [
        'intersect', 'step', 'nan', 'zero', 'time'])
...
class Window:
    ...
    **Examples**
    ...
    def __init__(self, w: Union[int, str, None]
                 = None, r: Union[int, str, None] = None):
        self.w = w
        self.r = w if r is None else r


def _check_window(series_length: int, window: Window):
    if series_length > 0 and isinstance(
            window.w,
            int) and isinstance(
            window.r,
            int):
...
def apply_ramp(x: pd.Series, window: Window) -> pd.Series:
    ...
    if isinstance(window.w, int) and window.w > len(
            x):  # does not restrict window size when it is a DataOffset
        return pd.Series([])
...
def normalize_window(x: Union[pd.Series,
                              pd.DataFrame],
                     window: Union[Window,
                                   int,
                                   str,
...
def plot_measure(asset_class: Optional[tuple] = None,
