...
from gs_quant.errors import (MqAuthenticationError, MqError, MqRequestError,
...
class GsSession(ContextBase):
    ...
    def __init__(
        ...
        self.http_adapter = requests.adapters.HTTPAdapter(
            pool_maxsize=100) if http_adapter is None else http_adapter
    ...
    def __request(
        ...
        url = '{}{}{}'.format(self.domain, '/' +
                              self.api_version if include_version else '', path)
        ...
        if method in ['GET', 'DELETE']:
            ...
            if is_dataframe or payload:
                kwargs['data'] = payload if isinstance(
                    payload, str) else msgpack.dumps(
                    payload, default=default) if use_msgpack else json.dumps(
                    payload, cls=JSONEncoder)
        else:
    ...
    def _get(self,
             path: str,
             payload: Optional[Union[dict,
                                     Base]] = None,
             request_headers: Optional[dict] = None,
             cls: Optional[type] = None,
             include_version: bool = True,
             timeout: int = DEFAULT_TIMEOUT) -> Union[Base,
                                                      tuple,
                                                      dict]:
        return self.__request(
            'GET',
    ...
    def use(
        ...
        environment_or_domain = environment_or_domain.name if isinstance(
            environment_or_domain, Environment) else environment_or_domain
        session = cls.get(
    ...
    def get(
        ...
        if client_id is not None:
            ...
            scopes = tuple(
                set(itertools.chain(scopes, cls.Scopes.get_default())))
            ...
            if is_gssso:
                try:
                    ...
                    raise MqUninitialisedError(
                        'This option requires gs_quant_internal to be installed')
            else:
            ...
            try:
                ...
                raise MqUninitialisedError(
                    'Must specify client_id and client_secret')
...
class OAuth2Session(GsSession):
    ...
    def _authenticate(self):
        ...
        if reply.status_code != 200:
            raise MqAuthenticationError(
                reply.status_code, reply.text, context=self.auth_url)
...
try:
    from gs_quant_internal.kerberos.session_kerberos import \
        KerberosSessionMixin
