...
class ScalarWithInfo(ResultInfo, metaclass=ABCMeta):
    ...
    def compose(components: Iterable):
        ...
        return SeriesWithInfo(
            pd.Series(index=pd.DatetimeIndex(dates).date, data=values),
            risk_key=risk_key,
            unit=unit,
            error=errors,
        )
        ...
        if isinstance(other, FloatWithInfo):
            if self.unit == other.unit:
                return FloatWithInfo(
                    combine_risk_key(self.risk_key, other.risk_key),
...
def combine_risk_key(key_1: RiskKey, key_2: RiskKey) -> RiskKey:
    ...
    def get_field_value(field_name: str):
        getattr(key_1, field_name) if getattr(key_1, field_name) == getattr(key_2, field_name) else None
