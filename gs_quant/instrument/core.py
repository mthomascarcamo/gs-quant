...
from gs_quant.base import InstrumentBase, QuotableBuilder, get_enum_value
from gs_quant.common import AssetClass, AssetType, XRef
from gs_quant.context_base import do_not_serialise
from gs_quant.markets import (HistoricalPricingContext, MarketDataCoordinate,
                              PricingCache, PricingContext)
from gs_quant.priceable import PriceableImpl
from gs_quant.risk import (DataFrameWithInfo, FloatWithInfo,
                           ResolvedInstrumentValues, RiskMeasure,
                           SeriesWithInfo)
from gs_quant.risk.results import (ErrorValue, MultipleRiskMeasureFuture,
                                   PricingFuture)
...
class Instrument(PriceableImpl, InstrumentBase, metaclass=ABCMeta):
    ...
    def __asset_class_and_type_to_instrument(cls):
        if not cls.__instrument_mappings:
            ...
            instrument_classes = [
                c for _, c in inspect.getmembers(
                    instrument_, inspect.isclass) if issubclass(
                    c, Instrument) and c is not Instrument]

            cls.__instrument_mappings[(
                AssetClass.Cash, AssetType.Currency)] = instrument_.Forward

            for clazz in instrument_classes:
                instrument = clazz.default_instance()
                cls.__instrument_mappings[(
                    instrument.asset_class, instrument.type)] = clazz
    ...
    def resolve(self,
                in_place: bool = True) -> Optional[Union[PriceableImpl,
                                                         PricingFuture,
                                                         dict]]:
        """
        ...
        **Examples**
        ...
        def handle_result(
                result: Optional[Union[ErrorValue, InstrumentBase]]) -> Optional[PriceableImpl]:
            ret = None if in_place else result
            if isinstance(result, ErrorValue):
                _logger.error(
                    'Failed to resolve instrument fields: ' +
                    result.error)
    ...
    def calc(self,
             ...
             fn=None) -> Union[DataFrameWithInfo,
                               ErrorValue,
                               FloatWithInfo,
                               PriceableImpl,
                               PricingFuture,
        ...
        **Examples**
        ...
        return future.result() if not (
            PricingContext.current.is_entered or PricingContext.current.is_async) else future
        ...
        if values:
            if issubclass(cls, QuotableBuilder):
                ...
                if '$type' in values:
                    ...
                    tdapi_cls = getattr(
                        tdapi, values['$type'].replace(
                            'Defn', 'Builder'))
                    if not tdapi_cls:
                        raise RuntimeError(
                            'Cannot resolve TDAPI type {}'.format(tdapi_cls))

                    return tdapi_cls.from_dict(values)

                asset_class_field = next(
                    (f for f in ('asset_class', 'assetClass') if f in values), None)
                ...
                default_type = Security if asset_type in [
                    None, "", "Security"] and asset_class in [
                    None, "", "Security"] else None
    ...
    @classmethod
    def from_asset_ids(
            cls, asset_ids: Tuple[str, ...]) -> Tuple[InstrumentBase, ...]:
        ...
        try:
            ...
            if not all(i.asset_class == asset_class and i.type ==
                       asset_type for i in instruments):
                raise ValueError(
                    f'Instrument(s) not all of type {cls.__name__}')
        except AttributeError:
            pass
    ...
    def compose(components: Iterable):
        return {
            c.risk_key.date if isinstance(
                c, ErrorValue) else c.resolution_key.date: c for c in components}
