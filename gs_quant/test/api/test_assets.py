...
def test_get_many_assets(mocker):
    ...
    inputs = EntityQuery(where=FieldFilterMap(**query), fields=None, asOfTime=as_of, limit=100)
...
def test_get_asset_positions_for_date(mocker):
    ...
    GsSession.current._get.assert_called_with("/assets/{id}/positions/{date}".format(id=marquee_id, date=position_date))
