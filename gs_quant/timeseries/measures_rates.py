...
from gs_quant.timeseries import ASSET_SPEC, GENERIC_DATE, BenchmarkType, MeasureDependency, plot_measure
...
class TdapiRatesDefaultsProvider:
    ...
    def get_swaption_parameter(self, currency, field, value=None):
        ...
        for entry in [
            self.defaults.get("CURRENCIES").get(currency_name)[0],
            self.defaults.get("COMMON"),
        ]:
            if field in entry:
...
SWAPTION_DEFAULTS = {
    "CURRENCIES": {
        ...
        "GBP": [
            {
                "benchmarkType": "LIBOR",
                "floatingRateOption": "GBP-LIBOR-BBA",
                "floatingRateTenor": ["6m", "3m"],
        ...
        "AUD": [
            {
                "benchmarkType": "BBR",
                "floatingRateOption": "AUD-BBR-BBSW",
                "floatingRateTenor": ["6m", "3m"],
        ...
        "JPY": [
            {
                "benchmarkType": "LIBOR",
                "floatingRateOption": "JPY-LIBOR-BBA",
                "floatingRateTenor": ["6m"],
...
CURRENCY_TO_SWAP_RATE_BENCHMARK = {
    ...
    "EUR": OrderedDict(
        [
            ("EURIBOR", "EUR-EURIBOR-TELERATE"),
            ("EONIA", "EUR-EONIA-OIS-COMPOUND"),
...
def _check_forward_tenor(forward_tenor) -> GENERIC_DATE:
    if isinstance(forward_tenor, datetime.date):
        ...
        _is_valid_relative_date_tenor(forward_tenor) or re.fullmatch("(imm[1-4]|frb[1-9]|ecb[1-6])", forward_tenor)
    ):
...
def _check_benchmark_type(currency, benchmark_type: Union[BenchmarkType, str]) -> BenchmarkType:
    ...
    if (
        isinstance(benchmark_type, BenchmarkType)
        and benchmark_type.value not in CURRENCY_TO_SWAP_RATE_BENCHMARK[currency.value].keys()
    ):
...
def _check_tenor_type(tenor_type: _SwapTenorType) -> _SwapTenorType:
    ...
    if tenor_type is None:
        ...
        raise MqValueError(
            "invalid tenor_type: " + tenor_type + " choose one among " + ", ".join([ch.value for ch in _SwapTenorType])
        )
...
def _get_term_struct_date(
    tenor: Union[str, datetime.datetime], index: datetime.datetime, business_day
) -> datetime.datetime:
...
@plot_measure(
    ...
    [
        MeasureDependency(
            id_provider=_currency_to_tdapi_swap_rate_asset,
...
@plot_measure(
    ...
    [
        MeasureDependency(
            id_provider=_currency_to_tdapi_swaption_rate_asset,
...
def _swaption_build_asset_query(
    ...
    for tenor in [termination_tenor, expiration_tenor]:
        if not _is_valid_relative_date_tenor(tenor):
            raise MqValueError("invalid tenor " + tenor + " for index: " + benchmark_type)
...
def _get_basis_swap_kwargs(
    ...
    for key, leg in legs_w_defaults.items():
        if not re.fullmatch("(\\d+)([bdwmy])", leg["floating_rate_tenor"]):
            raise MqValueError(
                "invalid floating rate tenor " + leg["floating_rate_tenor"] + " index: " + leg["benchmark_type"]
            )
...
@plot_measure(
    ...
    [
        MeasureDependency(
            id_provider=_currency_to_tdapi_basis_swap_rate_asset,
...
def swap_term_structure(
    ...
    *,
    ...
    _logger.debug(
        "where benchmark_type=%s, floating_rate_tenor=%s, %s=%s, " "pricing_location=%s",
        defaults["benchmark_type"],
        defaults["floating_rate_tenor"],
        tenor_type.value,
        tenor_dict["tenor"],
    ...
    with DataContext(start, end):
        q = GsDataApi.build_market_data_query(rate_mqids, QueryType.SWAP_RATE, source=source, real_time=real_time)
        _logger.debug("q %s", q)
        df = _market_data_timed(q)
