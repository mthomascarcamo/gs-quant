...
class GsRiskApi(RiskApi):
    ...
    def __url(cls, request: Union[RiskRequest, Iterable[RiskRequest]]):
        ...
        is_internal = any(
            hasattr(p.instrument, "_type")
            for p in chain.from_iterable(r.positions for r in (request if is_bulk else (request,)))
        )
            ...
            try:
                ...
                for result in calc_results:
                    if "error" in result:
                        results.put_nowait(
                            (
                                pending_requests.pop(result["requestId"]),
            ...
            try:
                ...
                if outstanding_request_ids:
                    ...
                    await asyncio.wait_for(
                        ws.send(json.dumps(outstanding_request_ids)),
                ...
                while pending_requests or not all_requests_dispatched:
                    ...
                    if result_listener in complete:
                        ...
                        if status == "E":
                            ...
                            try:
                                result = (
                                    msgpack.unpackb(base64.b64decode(result_str), raw=False)
                                    if cls.USE_MSGPACK
                    ...
                    if request_listener:
                        if request_listener in complete:
                            ...
                            if items:
                                ...
                                await asyncio.wait_for(
                                    ws.send(json.dumps(request_ids)),
                                    timeout=send_timeout,
                                )
