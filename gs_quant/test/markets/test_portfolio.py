...
from gs_quant.markets import BackToTheFuturePricingContext, HistoricalPricingContext, PricingContext
...
def test_historical_pricing(mocker):
    with MockCalc(mocker):
        ...
        assert (
            results[dt.date(2019, 10, 9)][risk.DollarPrice]["10y@a+1"]
    ...
    with MockCalc(mocker):
        ...
        with PricingContext(pricing_date=pricing_date):
            with BackToTheFuturePricingContext(
                dates=business_day_offset(pricing_date, [-1, 0, 1], roll="forward")
            ) as hpc:
                risk_key = hpc._PricingContext__risk_key(risk.DollarPrice, swap1.provider)
...
def test_nested_portfolios(mocker):
    ...
    assert portfolio.paths("USD-swap") == (
        PortfolioPath(2),
        PortfolioPath((1, 1, 0)),
        PortfolioPath((2, 1, 0)),
    )
