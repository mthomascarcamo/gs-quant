...
dummy_series2 = pd.Series([2790.37,
                           2700.06,
                           2695.95,
                           2633.08,
                           2637.72,
...
def ts_map():
    return {
        k: v for k,
        v in globals().items() if isinstance(
            v,
            types.FunctionType) and (
            getattr(
                v,
                'plot_function',
                False) or getattr(
...
def test_docstrings(ts_map):
    for k, v in ts_map.items():
        ...
        assert params == set(inspect.signature(
            v).parameters.keys()), 'all parameters documented'
...
def test_annotations(ts_map):
    for k, v in ts_map.items():
        ...
        assert set(inspect.signature(v).parameters.keys()) | {'return'} == set(
            annotations.keys()), 'specifies parameter types'
