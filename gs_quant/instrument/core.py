...
from gs_quant.risk import DataFrameWithInfo, FloatWithInfo, ResolvedInstrumentValues, RiskMeasure, SeriesWithInfo
...
class Instrument(PriceableImpl, InstrumentBase, metaclass=ABCMeta):
            ...
            if attr and isinstance(attr, property):
                ...
                if isinstance(resolved_inst, PricingFuture):
                    ...
                    resolved_inst.add_done_callback(
                        lambda inst_f: ret.set_result(object.__getattribute__(inst_f.result(), name))
                    )
                else:
    ...
    def calc(
        ...
        **Examples**
        ...
        """
        futures = {
            r: r.pricing_context.calc(self, r)
            for r in ((risk_measure,) if isinstance(risk_measure, RiskMeasure) else risk_measure)
        }
        future = (
            MultipleRiskMeasureFuture(self, futures)
            if len(futures) > 1
            else futures[risk_measure if isinstance(risk_measure, RiskMeasure) else risk_measure[0]]
        )
    ...
    def from_dict(cls, values: dict):
        if values:
            if issubclass(cls, QuotableBuilder):
                ...
                default_type = (
                    Security if asset_type in [None, "", "Security"] and asset_class in [None, "", "Security"] else None
                )

                instrument = cls.__asset_class_and_type_to_instrument().get(
                    (
                        get_enum_value(AssetClass, asset_class),
                        get_enum_value(AssetType, asset_type),
                    ),
                    default_type,
                )
