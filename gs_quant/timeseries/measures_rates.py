...
from gs_quant.target.common import Currency as CurrencyEnum
from gs_quant.target.common import PricingLocation
from gs_quant.timeseries import (ASSET_SPEC, GENERIC_DATE, BenchmarkType,
                                 MeasureDependency, plot_measure)
from gs_quant.timeseries.helper import _to_offset
from gs_quant.timeseries.measures import (ExtendedSeries, SwaptionTenorType,
                                          _asset_from_spec,
                                          _extract_series_from_df,
                                          _get_custom_bd, _market_data_timed,
...
class TdapiRatesDefaultsProvider():
    # flag to indicate that a given property should not  be included in asset
    # query
    EMPTY_PROPERTY = "null"
        ...
        for k, v in defaults.get("CURRENCIES").items():
            for e in v:
                benchmark_mappings[k] = {
                    e.get("benchmarkType"): e.get('floatingRateOption')}
    ...
    def get_floating_rate_option_for_benchmark(
            self, currency: CurrencyEnum, benchmark: str):
    ...
    def get_swaption_parameter(self, currency, field, value=None):
        ...
        for entry in [self.defaults.get("CURRENCIES").get(currency_name)[
                0], self.defaults.get("COMMON")]:
            if field in entry:
                value = entry[field][0] if isinstance(
                    entry[field], list) else entry[field]
        return value
...
def _currency_to_tdapi_swaption_rate_asset(asset_spec: ASSET_SPEC) -> str:
    ...
    try:
        result = swaptions_defaults_provider.get_swaption_parameter(
            bbid, "assetIdForAvailabilityCheck")
    except TypeError:
...
def _get_tdapi_rates_assets(allow_many=False, **kwargs) -> Union[str, list]:
    ...
    if len(assets) == 0 and (
            'asset_parameters_payer_rate_option' in kwargs):  # flip legs
...
def _check_benchmark_type(currency,
                          benchmark_type: Union[BenchmarkType,
                                                str]) -> BenchmarkType:
    if isinstance(benchmark_type, str):
        if benchmark_type.upper() in BenchmarkType.__members__:
            ...
            raise MqValueError('%s is not valid, pick one among ' +
...
def _check_clearing_house(
        clearing_house: Union[_ClearingHouse, str]) -> _ClearingHouse:
    if isinstance(
            clearing_house,
            str) and clearing_house.upper() in _ClearingHouse.__members__:
    ...
    if clearing_house is None:
        ...
        raise MqValueError('invalid clearing house: ' +
                           clearing_house +
                           ' choose one among ' +
...
def _check_tenor_type(tenor_type: _SwapTenorType) -> _SwapTenorType:
    if isinstance(tenor_type, str) and tenor_type.upper(
    ) in _SwapTenorType.__members__:
    ...
    if tenor_type is None:
        ...
        raise MqValueError('invalid tenor_type: ' +
                           tenor_type +
                           ' choose one among ' +
                           ', '.join([ch.value for ch in _SwapTenorType]))


def _check_term_structure_tenor(
        tenor_type: _SwapTenorType,
        tenor: str) -> Dict:
    ...
    return dict(tenor=tenor, tenor_to_plot=tenor_to_plot,
                tenor_dataset_field=tenor_dataset_field)


def _get_swap_leg_defaults(
        currency: CurrencyEnum,
        benchmark_type: BenchmarkType = None,
        floating_rate_tenor: str = None) -> dict:
    pricing_location = CURRENCY_TO_PRICING_LOCATION.get(
        currency, PricingLocation.LDN)
    ...
    if benchmark_type is None:
        if currency == CurrencyEnum.EUR:
            ...
            benchmark_type = BenchmarkType(
                str(list(CURRENCY_TO_SWAP_RATE_BENCHMARK[currency.value].keys())[0]))
...
def _get_basis_swap_csa_terms(
        curr: str,
        payer_benchmark: str,
        receiver_benchmark: str) -> dict:
...
def _get_swap_data(
    ...
    if currency.value not in CURRENCY_TO_SWAP_RATE_BENCHMARK.keys():
        raise NotImplementedError(
            'Data not available for {} swap rates'.format(
                currency.value))
    benchmark_type = _check_benchmark_type(currency, benchmark_type)

    clearing_house = _check_clearing_house(clearing_house)

    defaults = _get_swap_leg_defaults(
        currency, benchmark_type, floating_rate_tenor)

    if not (
        re.fullmatch(
            '(\\d+)([bdwmy])',
            swap_tenor) or re.fullmatch(
            '(frb[1-9]|ecb[1-6])',
            forward_tenor)):
    ...
    q = GsDataApi.build_market_data_query(
        [rate_mqid], query_type, source=source, real_time=real_time)
    _logger.debug('q %s', q)
    df = _market_data_timed(q)
    return df


def _get_term_struct_date(tenor: Union[str,
                                       datetime.datetime],
...
def swaption_premium(
        asset: Asset,
        expiration_tenor: str = None,
        termination_tenor: str = None,
...
def swaption_annuity(
        asset: Asset,
        expiration_tenor: str = None,
        termination_tenor: str = None,
        relative_strike: float = None,
        benchmark_type: str = None,
        floating_rate_tenor: str = None,
        clearing_house: str = None,
        *,
...
def _get_swaption_measure(
        ...
        strike_reference: [
            ...
            int] = None,
    clearing_house: str = None,
    start: str = DataContext.current.start_date,
    end: str = DataContext.current.end_date,
    source: str = None,
    real_time: bool = False,
...
def _swaption_build_asset_query(
        currency,
        benchmark_type=None,
        effective_date=None,
        ...
        clearingHouse=None):
    benchmark_type = swaptions_defaults_provider.get_swaption_parameter(
        currency, 'benchmarkType', benchmark_type)

    floating_rate_option = swaptions_defaults_provider.get_floating_rate_option_for_benchmark(
        currency, benchmark_type)
    if floating_rate_option is None:
        raise MqValueError(
            "Invalid benchmark type {}: cannot map benchmark_type with underlying rate for {}".format(
                benchmark_type, currency))
    floating_rate_tenor = swaptions_defaults_provider.get_swaption_parameter(
        currency, "floatingRateTenor", floating_rate_tenor)
    strike_reference = swaptions_defaults_provider.get_swaption_parameter(
        currency, 'strikeReference', strike_reference)

    termination_tenor = swaptions_defaults_provider.get_swaption_parameter(
        currency, 'terminationTenor', termination_tenor)
    effective_date = swaptions_defaults_provider.get_swaption_parameter(
        currency, 'effectiveDate', effective_date)
    expiration_tenor = swaptions_defaults_provider.get_swaption_parameter(
        currency, 'expirationTenor', expiration_tenor)
    ...
    clearingHouse = swaptions_defaults_provider.get_swaption_parameter(
        currency, 'clearingHouse', clearingHouse)
    query = dict(
        type='Swaption',
        asset_parameters_notional_currency=currency.name)
...
def _check_strike_reference(strike_reference):
    ...
    if isinstance(
        ...
        if strike_reference == 0:
            ...
            strike_reference = ('ATM%+f' %
                                strike_reference).rstrip('0').rstrip(".")
...
def swaption_vol_smile(
    ...
    _logger.debug(
        'where expiry=%s, tenor=%s',
        expiration_tenor,
        termination_tenor)
    currency = CurrencyEnum(asset.get_identifier(AssetIdentifier.BLOOMBERG_ID))
    location = swaptions_defaults_provider.get_swaption_parameter(
        currency, "pricingLocation")
    start, end = _range_from_pricing_date(location, pricing_date)
    df = _get_swaption_measure(
        asset,
        expiration_tenor=expiration_tenor,
        termination_tenor=termination_tenor,
        strike_reference=TdapiRatesDefaultsProvider.EMPTY_PROPERTY,
        source=source,
        query_type=QueryType.SWAPTION_VOL,
        benchmark_type=benchmark_type,
        floating_rate_tenor=floating_rate_tenor,
    ...
    if df.empty:
        ...
        df["strikeRelative"] = df["strikeRelative"].apply(
            lambda d: float(d.split("ATM")[1] if d != "ATM" else 0))
        latest = df.index.max()
...
@plot_measure((AssetClass.Cash,), (AssetType.Currency,),
              [MeasureDependency(id_provider=_currency_to_tdapi_swaption_rate_asset,
                                 query_type=QueryType.SWAPTION_VOL)])
def swaption_vol_term(
        asset: Asset,
        tenor_type: SwaptionTenorType,
        tenor: str,
    ...
    if tenor_type == SwaptionTenorType.OPTION_EXPIRY:
        ...
        df = _get_swaption_measure(
            asset,
            expiration_tenor=TdapiRatesDefaultsProvider.EMPTY_PROPERTY,
    ...
    if df.empty:
        ...
        df = df.assign(
            ...
            df[tenor_to_plot].map(_to_offset) +
            business_day -
            business_day)
...
def _get_basis_swap_kwargs(
        ...
        forward_tenor: Optional[GENERIC_DATE] = None,
        clearing_house: _ClearingHouse = None) -> dict:
    currency = CurrencyEnum(asset.get_identifier(AssetIdentifier.BLOOMBERG_ID))
    ...
    clearing_house = _check_clearing_house(clearing_house)
    spread_benchmark_type = _check_benchmark_type(
        currency, spread_benchmark_type)
    reference_benchmark_type = _check_benchmark_type(
        currency, reference_benchmark_type)

    # default benchmark types
    legs_w_defaults = dict()
    legs_w_defaults['spread'] = _get_swap_leg_defaults(
        currency, spread_benchmark_type, spread_tenor)
...
def swap_term_structure(
    ...
    tenor_dict = _check_term_structure_tenor(
        tenor_type=tenor_type, tenor=tenor)
    defaults = _get_swap_leg_defaults(
        currency, benchmark_type, floating_rate_tenor)
    ...
    if pricing_date is not None and pricing_date in list(
        ...
        raise MqValueError(
            'Specified pricing date is a holiday in {} calendar'.format(calendar))
    ...
    if df.empty:
        ...
        if isinstance(df, pd.Series):
            ...
            if col_to_plot == 'effectiveTenor':
                df = df[~df[col_to_plot].isin(
                    ['imm1', 'imm2', 'imm3', 'imm4'])]
            df['expirationDate'] = df[col_to_plot].apply(
...
def basis_swap_term_structure(
    ...
    *,
    ...
    _logger.debug('where spread_benchmark_type=%s, spread_tenor=%s,  reference_benchmark_type=%s, '
                  ...
                  kwargs['asset_parameters_receiver_rate_option'],
                  kwargs['asset_parameters_receiver_designated_maturity'],
                  kwargs[tenor_dict['tenor_dataset_field']],
                  tenor_dict['tenor'],
                  kwargs['pricing_location'])
