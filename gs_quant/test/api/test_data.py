...
from gs_quant.target.data import DataQuery, DataSetEntity, MarketDataVendor
...
def test_coordinates_data(mocker):
    ...
    mocker.patch.object(GsSession.current,
                        '_post',
                        side_effect=[{'responses': [{'data': bond_data}]},
    ...
    GsSession.current._post.assert_called_once_with(
        ...
        payload=MDAPIDataQuery(
            market_data_coordinates=test_coordinates,
            start_time=start,
            end_time=end,
...
def test_coordinate_data_series(mocker):
    ...
    bond_expected_series = pd.Series(
        index=bond_expected_frame.index,
        data=bond_expected_frame.value.values)
    swap_expected_series = pd.Series(
        index=swap_expected_frame.index,
...
def test_get_many_defns_api(mocker):
    ...
    GsSession.current._get.assert_called_with(
        '/data/datasets?limit=100', cls=DataSetEntity)
