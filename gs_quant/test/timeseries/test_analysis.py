...
def test_last_value():
    ...
    x = pd.Series([1.0, 2.0, 3.0, 4.0], index=(
...
def test_lag():
    ...
    result = lag(x, '1m')
    expected = pd.Series([1.0, 2.0, 3.0, 4.0], index=pd.date_range(
        "2019-01-31", periods=4, freq="D"))
    assert_series_equal(result, expected, obj="Lag 1m")

    result = lag(x, '2d', LagMode.TRUNCATE)
    expected = pd.Series([1.0, 2.0], index=pd.date_range(
        "2019-01-03", periods=2, freq="D"))
    ...
    z = pd.Series([10, 11, 12], index=pd.date_range(
        '2020-02-28', periods=3, freq='D'))
    result = lag(z, '2y')
    expected = pd.Series([10, 12], index=pd.date_range(
        '2022-02-28', periods=2, freq='D'))
