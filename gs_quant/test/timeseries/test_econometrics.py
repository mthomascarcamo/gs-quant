...
def test_returns():
    ...
    expected = pd.Series(
        [np.nan, np.nan, 0.0302, -0.0004, -0.0200, 0.0200], index=dates)
    assert_series_equal(
        result,
        expected,
        obj="Simple returns",
    ...
    expected = pd.Series([np.nan, 0.009950, 0.019803, -
                          0.020203, 0.0, 0.019803], index=dates)
    assert_series_equal(
        result,
        expected,
        obj="Logarithmic returns",
        check_less_precise=True)

    result = returns(x, 2, Returns.LOGARITHMIC)
    expected = pd.Series(
        [np.nan, np.nan, 0.029753, -0.0004, -0.020203, 0.019803], index=dates)
    assert_series_equal(
        result,
        expected,
        obj="Logarithmic returns",
    ...
    expected = pd.Series(
        [np.nan, 1.0, 2.02, -2.0604, 0.0, 2.019192], index=dates)
    assert_series_equal(
        result,
        expected,
        obj="Absolute returns",
        check_less_precise=True)

    result = returns(x, 2, Returns.ABSOLUTE)
    expected = pd.Series(
        [np.nan, np.nan, 3.02, -0.0404, -2.0604, 2.019192], index=dates)
    assert_series_equal(
        result,
        expected,
        obj="Absolute returns",
...
def test_prices():
    ...
    result = prices(r)
    expected = pd.Series([1.0, 1.01, 1.0302, 1.009596,
                          1.009596, 1.02978792], index=dates)
    assert_series_equal(result, expected, obj="Simple price series default")

    result = prices(r, 100)
    expected = pd.Series([100.0, 101, 103.02, 100.9596,
                          100.9596, 102.978792], index=dates)
    assert_series_equal(result, expected, obj="Simple price series default")
    ...
    assert_series_equal(
        result,
        expected,
        obj="Simple price series",
        check_less_precise=True)

    r = pd.Series([np.nan, 0.009950, 0.019803, -
                   0.020203, 0.0, 0.019803], index=dates)
...
def test_index():
    ...
    result = index(x)
    expected = pd.Series(
        [1.000, 1.010, 1.005, 1.015, 1.010, 1.005], index=dates)
...
def test_annualize():
    ...
    daily_series = pd.Series(
        [0.01, 0.02, -0.01, 0.03, 0, -0.01], index=daily_dates)
    ...
    weekly_series = pd.Series(
        [0.01, 0.02, -0.01, 0.03, 0, -0.01], index=weekly_dates)
    ...
    semi_monthly_series = pd.Series(
        [0.01, 0.02, -0.01, 0.03, 0, -0.01], index=semi_monthly_dates)
    ...
    monthly_series = pd.Series(
        [0.01, 0.02, -0.01, 0.03, 0, -0.01], index=monthly_dates)
    ...
    quarterly_series = pd.Series(
        [0.01, 0.02, -0.01, 0.03, 0, -0.01], index=quarterly_dates)
    ...
    annual_series = pd.Series(
        [0.01, 0.02, -0.01, 0.03, 0, -0.01], index=annual_dates)
    ...
    invalid_series = pd.Series(
        [0.01, 0.02, -0.01, 0.03, 0, -0.01], index=invalid_dates)
...
def test_volatility():
    ...
    x = pd.Series([100.0, 101, 103.02, 100.9596, 100.9596,
                   102.978792], index=daily_dates)
    ...
    assert_series_equal(
        pd.Series(),
        expected,
        obj="Volatility strdate too large for series")
...
def test_correlation():
    ...
    x = pd.Series([100.0, 101, 103.02, 100.9596, 100.9596,
                   102.978792], index=daily_dates)
    y = pd.Series([100.0, 101, 103.02, 100.9596, 100.9596,
                   102.978792], index=daily_dates)

    result = correlation(x, y)
    expected = pd.Series(
        [np.nan, np.nan, 1.0, 1.0, 1.0, 1.0], index=daily_dates)

    assert_series_equal(result, expected, check_less_precise=True)

    y = pd.Series([100.0, 102.0, 104.0, 101.0,
                   100.95, 100.0], index=daily_dates)

    result = correlation(x, y)
    expected = pd.Series([np.nan, np.nan, -1.0, 0.969025,
                          0.969254, 0.706042], index=daily_dates)
    ...
    expected = pd.Series(
        [np.nan, np.nan, -1.0, 1.0, np.nan, -1.0], index=daily_dates)
    assert_series_equal(result, expected, check_less_precise=True)
    ...
    assert_series_equal(
        result,
        expected,
        obj="Correlation strdate as window with too large of window")
...
def test_beta():
    ...
    result = beta(x, y)
    expected = pd.Series([np.nan, np.nan, np.nan, 0.718146,
                          0.718919, 0.572201], index=daily_dates)
    assert_series_equal(result, expected, check_less_precise=True)
    ...
    result = beta(x, y, '2d')
    expected = pd.Series([np.nan, 0.8255252918287954,
                          np.nan, -2.24327163719368], index=daily_dates[2:])
...
def test_sharpe_ratio():
    ...
    numpy.testing.assert_almost_equal(
        actual.values, er_df['SR'].values, decimal=5)
    ...
    actual = _get_ratio(
        ...
        day_count_convention=DayCountConvention.ACTUAL_360)
    numpy.testing.assert_almost_equal(
        actual.values, er_df['SR10'].values[10:], decimal=5)
...
def test_arima_fit():
    ...
    for train_size in train_size_values:
        ...
        assert (
            isclose(
                transformed_test_df['High'][3],
                (arima.best_params['High'].const +
                 test_df_high[2] *
                 arima.best_params['High'].ar_coef[0] +
                 test_df_high[1] *
                 arima.best_params['High'].ar_coef[1]),
                abs_tol=1e-8))
        assert (
            isclose(
                transformed_test_df['High'][4],
                (arima.best_params['High'].const +
                 test_df_high[3] *
                 arima.best_params['High'].ar_coef[0] +
                 test_df_high[2] *
        ...
        assert (
            isclose(
                transformed_test_df['Low'][4],
                (arima.best_params['Low'].const +
                 test_df_low[3] *
                 arima.best_params['Low'].ar_coef[0] +
                 test_df_low[2] *
                 arima.best_params['Low'].ar_coef[1]),
                abs_tol=1e-8))
        assert (
            isclose(
                transformed_test_df['Low'][5],
                (arima.best_params['Low'].const +
                 test_df_low[4] *
                 arima.best_params['Low'].ar_coef[0] +
                 test_df_low[3] *
        ...
        assert (
            isclose(
                transformed_test_df['Close'][3],
                (arima.best_params['Close'].const +
                 test_df_close[2] *
                 arima.best_params['Close'].ar_coef[0] +
                 test_df_close[1] *
                 arima.best_params['Close'].ar_coef[1]),
                abs_tol=1e-8))
        assert (
            isclose(
                transformed_test_df['Close'][4],
                (arima.best_params['Close'].const +
                 test_df_close[3] *
                 arima.best_params['Close'].ar_coef[0] +
                 test_df_close[2] *
    ...
    assert (
        isclose(
            transformed_test_series['High'][3],
            (arima.best_params['High'].const +
             test_series_high[2] *
             arima.best_params['High'].ar_coef[0] +
             test_series_high[1] *
    ...
    assert (
        isclose(
            transformed_test_series['High'][4],
            (arima.best_params['High'].const +
             test_series_high[3] *
             arima.best_params['High'].ar_coef[0] +
             test_series_high[2] *
    ...
    zero_resid[:] = 0
    new_arima.best_params = {
        'High': econometrics.ARIMABestParams(
            p=0,
            q=0,
            d=0,
            const=0,
