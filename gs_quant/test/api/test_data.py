...
def test_coordinates_data(mocker):
    ...
    mocker.patch.object(
        ...
        side_effect=[
            ...
            {"responses": [{"data": bond_data}, {"data": swap_data}]},
            {"responses": [{"data": bond_data}, {"data": swap_data}]},
        ],
    )
    ...
    coords_data_result = GsDataApi.coordinates_data(
        coordinates=test_coordinates, start=start, end=end, as_multiple_dataframes=True
    )
...
def test_get_many_coordinates(mocker):
    coordinates = [
        {"id": "MC123", "name": "A_B_C_D_E.F1"},
        {"id": "MC123", "name": "A_B_C_D_E.F2"},
    ]
    mocker.patch.object(
        GsSession.__class__,
        "default_value",
