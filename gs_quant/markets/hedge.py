...
class Hedge:
    ...
    def find_optimal_hedge(
            hedge_query: dict, hyperparams: dict, metric: str) -> Union[dict, float]:
        """
        ...
        _logger.info(
            f'We are trying to {optimization_type} {metric} and will return the optimized hedge & '
            f'metric value...')
        hyperparam_grid = [(x, y) for x in hyperparams['Concentration']
                           for y in hyperparams['Diversity']]
        for pair in hyperparam_grid:
            ...
            _logger.info(
                f'Current Hedge is using the following values for Concentration/Diversity: {pair}')
            curr_results, curr_pair = results['result']['hedge'], pair
            hedge_results[curr_results[metric]] = (curr_results, curr_pair)
            _logger.info(
                f'Current Hedge value for {metric}: {curr_results[metric]*100:.3}%')
        optimized_metric = min(
            hedge_results.keys()) if optimization_type == 'minimize' else max(
            hedge_results.keys())
        optimized_hedge, optimized_hyperparams = hedge_results[
            optimized_metric][0], hedge_results[optimized_metric][1]
    ...
    def plot_weights_against_number_of_assets(
            hedge_query: dict, hyperparams: dict, figsize: tuple = (
            18, 9)) -> matplotlib.figure.Figure:
        """
        ...
        try:
            for i in range(len(hyperparams[hyperparam_to_plot])):
                ...
                hedge_query['parameters'] = hedge_params
                results = GsHedgeApi.calculate_hedge(hedge_query)
                portfolio, weights, asset_numbers = Hedge.construct_portfolio_weights_and_asset_numbers(
                    results)
                x_ind = np.arange(len(asset_numbers))
                bar = ax.bar(x_ind, weights, align='center', alpha=0.6)
                lines.append(bar)
            plt.legend(lines, [hyperparam_to_plot + ' Percentage: ' + str(i)
                               for i in hyperparams[hyperparam_to_plot]], prop={'size': 18})
            plt.xlabel('Number of Assets', size=13)
            plt.ylabel('Weights', size=13)
            plt.title(
                'Analyzing the effects of the ' +
                hyperparam_to_plot +
                ' hyperparameter on a hedge',
                size=22)
            plt.show()
    ...
    def create_transaction_cost_data_structures(
        ...
        for date in backtest_dates:
            data = thomson_reuters_eod_data.get_data(
                date, date, assetId=portfolio_asset_ids)
            prices_df = prices_df.append(data)
        for asset_id in portfolio_asset_ids:
            id_prices_map[asset_id] = list(
                prices_df.loc[prices_df['assetId'] == asset_id]['closePrice'])

        # Create list representing notional of each day in transaction_cost_days and map asset_ids to notional of each
        # asset on each day
        id_to_notional_map = {}
        notionals_assets = [abs(np.asarray(id_prices_map[asset_id]) *
                                id_quantity_map[asset_id]) for asset_id in portfolio_asset_ids]
        ...
        id_to_weight_map = {}
        for idx, asset_id in enumerate(portfolio_asset_ids):
            id_to_weight_map[asset_id] = [i / j for i,
                                          j in zip(id_to_notional_map[portfolio_asset_ids[idx]],
    ...
    def compute_tcosts(
        ...
        for idx, date in enumerate(backtest_dates):
            ...
            for asset_id in portfolio_asset_ids:
                ...
                notional_on_the_day, curr_weights = asset_notionals[
                    asset_id][idx], asset_weights[asset_id][idx]
                notional_to_trade = Hedge.compute_notional_traded(
                    notional_on_the_day, prev_weights, curr_weights)
                transaction_cost = Hedge.t_cost(
                    basis_points, notional_to_trade)
    ...
    @staticmethod
    def plot_transaction_costs_of_rebalancing(
        ...
        ax_costs = cumulative_transaction_costs.plot(
            figsize=figsize, linewidth=3)
        ...
        plt.title(
            'Cumulative Costs to Rebalance Weights-Based Performance Hedger',
            size=22)
        plt.legend(
            labels=['Weights-Based Performance Hedger'],
            prop={
                'size': 18})
