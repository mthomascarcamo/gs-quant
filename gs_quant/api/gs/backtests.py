...
class GsBacktestApi:
    ...
    def get_many_backtests(cls,
        ...
        return GsSession.current._get(
            '/backtests?{query}'.format(query=query_string), cls=Backtest)['results']

    @classmethod
    def get_backtest(cls, backtest_id: str) -> Backtest:
        return GsSession.current._get(
            '/backtests/{id}'.format(id=backtest_id), cls=Backtest)
    ...
    def delete_backtest(cls, backtest_id: str) -> dict:
        return GsSession.current._delete(
            '/backtests/{id}'.format(id=backtest_id))

    @classmethod
    def get_results(cls, backtest_id: str) -> Tuple[BacktestResult, ...]:
        return GsSession.current._get(
            '/backtests/results?id={id}'.format(id=backtest_id))['backtestResults']
    ...
    def get_comparison_results(
        ...
        result = GsSession.current._get(
            '/backtests/results?{query}'.format(query=query_string))
        return result['backtestResults'], result['comparisonResults']

    @classmethod
    def schedule_backtest(cls, backtest_id: str) -> dict:
        return GsSession.current._post(
            '/backtests/{id}/schedule'.format(id=backtest_id))

    @classmethod
    def run_backtest(
            cls,
            backtest: Backtest,
            correlation_id: str = None) -> BacktestResult:
        """
    ...
    def calculate_position_risk(
            cls, backtestRiskRequest: BacktestRiskRequest) -> dict:
        request_headers = {'Content-Type': 'application/json;charset=utf-8'}
        return GsSession.current._post(
            '/backtests/calculate-position-risk',
            backtestRiskRequest,
            request_headers=request_headers)
    ...
    def get_ref_data(cls) -> BacktestRefData:
        return GsSession.current._get(
            '/backtests/refData', cls=BacktestRefData)
