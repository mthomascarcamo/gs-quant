...
class Entity(metaclass=ABCMeta):
    ...
    @classmethod
    def get(cls, id_value: str, id_type: EntityIdentifier) -> Optional["Entity"]:
        if id_type.value == "MQID":
            ...
            result = get(
                GsSession.current._get(
                    f"/{cls._entity_to_endpoint[cls.entity_type()]}?{id_type.value.lower()}={id_value}"
                ),
                "results.0",
            )
        if result:
    ...
    @classmethod
    def _get_entity_from_type(cls, entity: Dict):
        id_ = entity.get("id")
