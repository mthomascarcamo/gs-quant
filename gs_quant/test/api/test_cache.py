...
def test_cache_subset(mocker):
    ...
    risk_key = PricingContext(pricing_date=dt.date(2019, 10, 9))._PricingContext__risk_key(risk.Price, ir_swap.provider)
        ...
        with PricingContext(pricing_date=date) as pc:
            for risk_measure in (risk.Price, risk.IRDelta, risk.IRVega):
                val = PricingCache.get(
                    pc._PricingContext__risk_key(risk_measure, ir_swaption.provider),
                    ir_swaption,
                )
