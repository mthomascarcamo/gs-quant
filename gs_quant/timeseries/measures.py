...
def implied_correlation(
    ...
    *,
    ...
    def calculate_vol(group):
        ...
        return pd.Series(
            [sum(parts), sum(map(lambda x: pow(x, 2), parts))],
            index=["first", "second"],
        )
...
def average_realized_volatility(
    ...
    *,
    ...
    if top_n_of_index is not None:
        ...
        series = (
            ExtendedSeries(
                pd.concat(weighted_vols, axis=1).sum(1, min_count=1),
...
@plot_measure(
    ...
    [
        MeasureDependency(
            id_provider=currency_to_default_benchmark_rate,
...
def cap_floor_atm_fwd_rate(
    asset: Asset, expiration_tenor: str, *, source: str = None, real_time: bool = False
) -> Series:
    """
...
@plot_measure(
    ...
    [
        MeasureDependency(
            id_provider=currency_to_inflation_benchmark_rate,
            query_type=QueryType.INFLATION_SWAP_RATE,
        )
    ],
)
def zc_inflation_swap_rate(
    asset: Asset, termination_tenor: str, *, source: str = None, real_time: bool = False
) -> Series:
    """
...
@plot_measure(
    (AssetClass.FX,),
    (AssetType.Cross,),
    [MeasureDependency(id_provider=cross_to_usd_based_cross, query_type=QueryType.GIR_FX_FORECAST)],
)
def gir_fx_forecast(
    asset: Asset,
    ...
    *,
    ...
    if df.empty:
        ...
        try:
            ...
            series = ExtendedSeries(
                sqrt((t2_month * lg ** 2 - t1_month * sg ** 2) / _tenor_to_month(tenor)),
                name="forwardVol",
            )
    series.dataset_ids = dataset_ids
    return series
...
def _process_forward_vol_term(asset: Asset, df: pd.DataFrame, name: str) -> pd.Series:
    if df.empty:
        ...
        series = sqrt(
            (df["tenorInMonth"] * df[name] ** 2 - df["tenorInMonth"].shift(1) * df[name].shift(1) ** 2)
...
def fair_price(asset: Asset, tenor: str = None, *, source: str = None, real_time: bool = False) -> pd.Series:
    """'
...
def get_contract_range(start_contract_range, end_contract_range, timezone):
    if timezone:
        df = pd.date_range(
            ...
            end=end_contract_range + datetime.timedelta(days=1),
            freq="H",
            closed="left",
            tz=timezone,
        ).to_frame()
...
def bucketize_price(
    ...
    *,
    ...
    # in local time and then converted to UTC time
    start_time = datetime.datetime.combine(start_date, datetime.datetime.min.time(), tzinfo=from_zone).astimezone(
        to_zone
    )
    ...
    *,
    ...
    if level_type == "relative":
        spot_df = ds.get_data(
            ...
            meetingNumber=0,
            start=CENTRAL_BANK_WATCH_START_DATE,
        ).rename(columns={"value": "spotValue"})
        if spot_df.empty:
            raise MqValueError("no spot data returned to rebase")
        joined_df = cbw_df.merge(
            spot_df,
            on=["date", "assetId", "rateType", "location", "valuationDate"],
            how="inner",
        )
...
def realized_volatility(
    ...
    *,
    ...
    """
    q = GsDataApi.build_market_data_query([asset.get_marquee_id()], QueryType.SPOT, source=source, real_time=real_time)
    df = _market_data_timed(q)
...
def esg_aggregate(
    ...
    *,
    ...
    if metric == EsgMetric.ENVIRONMENTAL_SOCIAL_DISCLOSURE:
        ...
        query_metric = "{metric}{unit}".format(
            metric=inflection.camelize(metric.value, False),
...
def gir_rating(asset: Asset, metric: _RatingMetric, *, source: str = None, real_time: bool = False) -> Series:
    ...
    _logger.debug(
        "where assetId=%s, metric=%s, query_type=%s",
        mqid,
        metric.value,
        query_type.value,
    )
...
@plot_measure(
    (AssetClass.FX,),
    (AssetType.Cross,),
    [MeasureDependency(id_provider=cross_to_usd_based_cross, query_type=QueryType.GIR_GSDEER_GSFEER)],
)
...
@plot_measure(
    ...
    [
        ...
        QueryType.FINANCIAL_RETURNS_SCORE,
        QueryType.MULTIPLE_SCORE,
        QueryType.INTEGRATED_SCORE,
    ],
)
...
@plot_measure(
    ...
    [QueryType.GIR_COMMODITIES_FORECAST],
)
