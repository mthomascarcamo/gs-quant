...
from gs_quant.markets import (HistoricalPricingContext, OverlayMarket,
                              PricingContext)
from gs_quant.priceable import PriceableImpl
from gs_quant.risk import RiskMeasure
from gs_quant.risk.results import (CompositeResultFuture, PortfolioPath,
                                   PortfolioRiskResult, PricingFuture)
...
class Portfolio(PriceableImpl):
    ...
    def __contains__(self, item):
        if isinstance(item, PriceableImpl):
            return any(
                item in p.__priceables_lookup for p in self.all_portfolios + (self,))
        elif isinstance(item, str):
            return any(
                item in p.__priceables_by_name for p in self.all_portfolios + (self,))
        else:
            return False
    ...
    @priceables.setter
    def priceables(self,
                   priceables: Union[PriceableImpl,
                                     Iterable[PriceableImpl]]):
        self.__priceables = (priceables,) if isinstance(
            priceables, PriceableImpl) else tuple(priceables)
    ...
    def instruments(self) -> Tuple[Instrument, ...]:
        return tuple(
            unique_everseen(
                i for i in self.__priceables if isinstance(
                    i, Instrument)))

    @property
    def all_instruments(self) -> Tuple[Instrument, ...]:
        instr = chain(self.instruments, chain.from_iterable(
            p.all_instruments for p in self.all_portfolios))
    ...
    def __from_internal_positions(id_type: str, positions_id):
        instruments = GsPortfolioApi.get_instruments_by_position_type(
            id_type, positions_id)
        return Portfolio(instruments, name=positions_id)

    @staticmethod
    def from_eti(eti: str):
        return Portfolio.__from_internal_positions(
            'ETI', eti.replace(',', '%2C'))
    ...
    def from_asset_id(asset_id: str, date=None):
        ...
        response = GsAssetApi.get_asset_positions_for_date(
            asset_id, date) if date else GsAssetApi.get_latest_positions(asset_id)
    ...
    def from_portfolio_id(portfolio_id: str, date=None):
        ...
        response = GsPortfolioApi.get_positions_for_date(
            portfolio_id, date) if date else GsPortfolioApi.get_latest_positions(portfolio_id)
        response = response[0] if isinstance(response, tuple) else response
        positions = response.positions if isinstance(
            response, PositionSet) else response['positions']
        instruments = GsAssetApi.get_instruments_for_positions(positions)
    ...
    def save(self, overwrite: Optional[bool] = False):
        ...
        if self.__id:
            ...
            try:
                ...
                from gs_quant.target.portfolios import \
                    Portfolio as MarqueePortfolio
    ...
    def from_frame(cls, data: pd.DataFrame, mappings: dict = None):
        ...
        for row in (
            r for _, r in data.iterrows() if any(
                v for v in r.values if v is not None)):
            instrument = None
            for init_keys in (('asset_class', 'type'), ('$type',)):
                ...
                if len(init_keys) == len(init_values):
                    ...
                    instrument = instrument.from_dict(
                        {p: get_value(row, p) for p in instrument.properties()})
                    break
    ...
    def from_csv(
        ...
        data = pd.read_csv(csv_file,
                           skip_blank_lines=True).replace({np.nan: None})
    ...
    def append(self,
               priceables: Union[PriceableImpl,
                                 Iterable[PriceableImpl]]):
        self.priceables += ((priceables,
                             ) if isinstance(priceables,
                                             PriceableImpl) else tuple(priceables))

    def pop(self, item) -> PriceableImpl:
        priceable = self[item]
        self.priceables = [
            inst for inst in self.instruments if inst != priceable]
        return priceable
            ...
            for priceable in portfolio.priceables:
                if isinstance(priceable, Portfolio):
                    ...
                    records.append(
                        dict(
                            chain(
                                as_dict.items(), (('instrument', priceable), ('portfolio', portfolio.name)))))

            return records

        df = pd.DataFrame.from_records(to_records(self)).set_index([
            'portfolio', 'instrument'])
        all_columns = df.columns.to_list()
        columns = sorted(
            c for c in all_columns if c not in (
    ...
    def all_paths(self) -> Tuple[PortfolioPath, ...]:
        ...
        while stack:
            ...
            for idx, priceable in enumerate(portfolio.__priceables):
                path = parent + \
                    PortfolioPath(idx) if parent is not None else PortfolioPath(idx)
                if isinstance(priceable, Portfolio):
    ...
    def paths(self, key: Union[str, PriceableImpl]
              ) -> Tuple[PortfolioPath, ...]:
        if not isinstance(key, (str, Instrument, Portfolio)):
            raise ValueError('key must be a name or Instrument or Portfolio')

        idx = self.__priceables_by_name.get(key) if isinstance(
            key, str) else self.__priceables_lookup.get(key)
        paths = tuple(PortfolioPath(i) for i in idx) if idx else ()

        for path, porfolio in (
            (PortfolioPath(i), p) for i, p in enumerate(
                self.__priceables) if isinstance(
                p, Portfolio)):
    ...
    def resolve(self,
                in_place: bool = True) -> Optional[Union[PricingFuture,
                                                         PriceableImpl,
                                                         dict]]:
        ...
        if not in_place:
            ret = {} if isinstance(
                ...
                name=self.name)
            result_future = PricingFuture() if not isinstance(pricing_context,
                                                              PricingContext) or pricing_context.is_async or pricing_context.is_entered else None
                ...
                if isinstance(ret, Portfolio):
                    ...
                    for future in futures:
                        for date, priceable in future.result().items():
                            priceables_by_date.setdefault(
                                date, []).append(priceable)

                    for date, priceables in priceables_by_date.items():
                        if any(
                            p for p in priceables if not isinstance(
                                p, PriceableImpl)):
                            _logger.error(
                                f'Error resolving on {date}, skipping that date')
                        else:
            ...
            for result in results:
                if market_data is not None:
                    ...
                    for market in result.values():
                        update_market_data(
                            overlay_markets.setdefault(
                                market.base_market, {}), market.market_data)

            if market_data:
                ret = OverlayMarket(
                    base_market=results[0].base_market,
                    market_data=market_data)
            else:
                ret = {
                    base_market.date: OverlayMarket(
                        base_market=base_market,
                        market_data=market_data) for base_market,
                    market_data in overlay_markets.items()}
    ...
    def calc(self,
             risk_measure: Union[RiskMeasure,
                                 Iterable[RiskMeasure]],
             fn=None) -> PortfolioRiskResult:
        with self.__pricing_context:
            return PortfolioRiskResult(
                self, (risk_measure,) if isinstance(
                    risk_measure, RiskMeasure) else risk_measure, [
                    p.calc(
                        risk_measure, fn=fn) for p in self.__priceables])
