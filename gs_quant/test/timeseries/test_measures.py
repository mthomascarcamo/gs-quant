...
from gs_quant.markets.securities import AssetClass, Cross, Currency, Index, SecurityMaster, Stock, Swap
...
def map_identifiers_default_mocker(
...
def map_identifiers_swap_rate_mocker(
    input_type: Union[GsIdType, str],
    output_type: Union[GsIdType, str],
    ids: IdList,
    as_of: dt.datetime = None,
...
def map_identifiers_inflation_mocker(
    input_type: Union[GsIdType, str],
    output_type: Union[GsIdType, str],
    ids: IdList,
    as_of: dt.datetime = None,
...
def map_identifiers_cross_basis_mocker(
    input_type: Union[GsIdType, str],
    output_type: Union[GsIdType, str],
    ids: IdList,
    as_of: dt.datetime = None,
...
def test_get_term_struct_date(mocker):
    ...
    assert datetime.datetime(2021, 7, 30, 0, 0) == tm_rates._get_term_struct_date(
...
def mock_commod(_cls, _q):
    ...
    df = MarketDataResponseFrame(
        data=d,
        index=pd.date_range("2019-05-01", periods=31, freq="H", tz=timezone("UTC")),
    )
...
def mock_fx_empty(_cls, _q):
    d = {"strikeReference": [], "relativeStrike": [], "impliedVolatility": []}
...
def mock_eq_norm(_cls, _q):
    d = {"relativeStrike": [-4.0, 4.0, 0], "impliedVolatility": [5, 1, 2]}
    df = MarketDataResponseFrame(data=d, index=_index * 3)
    df.dataset_ids = _test_datasets
    return df
...
def mock_eq_spot(_cls, _q):
    d = {"relativeStrike": [0.75, 1.25, 1.0], "impliedVolatility": [5, 1, 2]}
    df = MarketDataResponseFrame(data=d, index=_index * 3)
    df.dataset_ids = _test_datasets
    return df
...
def mock_inc(_cls, _q):
    d = {"relativeStrike": [0.25, 0.75], "impliedVolatility": [5, 1]}
...
def mock_meeting_absolute():
    data_dict = MarketDataResponseFrame(
        {
            "date": [datetime.date(2019, 12, 6), datetime.date(2019, 12, 6)],
...
def mock_index_positions_data(asset_id, start_date, end_date, fields=None, position_type=None):
    return [
        {
...
def mock_rating(_cls, _q):
    d = {"rating": ["Buy", "Sell", "Buy", "Neutral"], "convictionList": [1, 0, 0, 0]}
    df = MarketDataResponseFrame(
        data=d,
...
def test_gir_fx_forecast():
    ...
    assert_series_equal(
        pd.Series([1.1, 1.1, 1.1], index=_index * 3, name="girFxForecast"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
...
def test_gir_fx_forecast_inverse():
    ...
    actual = tm.gir_fx_forecast(mock, "3m")
    assert_series_equal(
        pd.Series([1 / 1.1, 1 / 1.1, 1 / 1.1], index=_index * 3, name="girFxForecast"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    replace.restore()
...
def test_impl_corr():
    ...
    assert_series_equal(
        pd.Series([5, 1, 2], index=_index * 3, name="impliedCorrelation"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
...
def test_real_corr():
    ...
    assert_series_equal(
        pd.Series([3.14, 2.71828, 1.44], index=_index * 3),
        pd.Series(actual),
        check_names=False,
    )
    assert actual.dataset_ids == _test_datasets
...
def test_cds_implied_vol():
    ...
    assert_series_equal(
        pd.Series([5, 1, 2], index=_index * 3, name="impliedVolatilityByDeltaStrike"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
...
def test_avg_impl_vol():
    ...
    assert_series_equal(
        pd.Series([5, 1, 2], index=_index * 3, name="averageImpliedVolatility"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    ...
    df1 = pd.DataFrame(
        data={"impliedVolatility": [1, 2, 3], "assetId": ["MA1", "MA1", "MA1"]},
        index=pd.date_range(start="2020-01-01", periods=3),
    )
    df2 = pd.DataFrame(
        data={"impliedVolatility": [2, 3, 4], "assetId": ["MA2", "MA2", "MA2"]},
        index=pd.date_range(start="2020-01-01", periods=3),
    )
    df3 = pd.DataFrame(
        data={"impliedVolatility": [2, 5], "assetId": ["MA3", "MA3"]},
...
def test_avg_realized_vol():
    ...
    assert_series_equal(
        pd.Series([5, 1, 2], index=_index * 3, name="averageRealizedVolatility"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    ...
    df1 = pd.DataFrame(
        data={"spot": [1, 2, 3], "assetId": ["MA1", "MA1", "MA1"]},
        index=pd.date_range(start="2020-01-01", periods=3),
    )
    df2 = pd.DataFrame(
        data={"spot": [2, 3, 4], "assetId": ["MA2", "MA2", "MA2"]},
        index=pd.date_range(start="2020-01-01", periods=3),
    )
    df3 = pd.DataFrame(
        data={"spot": [2, 5], "assetId": ["MA3", "MA3"]},
        index=pd.date_range(start="2020-01-01", periods=2),
    )
...
def test_avg_impl_var():
    ...
    assert_series_equal(
        pd.Series([5, 1, 2], index=_index * 3, name="averageImpliedVariance"),
        pd.Series(actual),
    )
...
def test_swap_term_structure():
    ...
    bd_mock.return_value = pd.DataFrame(
        data=dict(date="2020-04-10", exchange="NYC", description="Good Friday"),
    ...
    expected = tm.ExtendedSeries(
        [1, 2, 3, 4],
        index=pd.to_datetime(["2020-01-01", "2021-01-01", "2021-12-31", "2022-12-30"]),
    )
    expected.dataset_ids = _test_datasets
...
def test_cap_floor_atm_fwd_rate():
    ...
    assert_series_equal(
        pd.Series([1, 2, 3], index=_index * 3, name="capFloorAtmFwdRate"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    with pytest.raises(NotImplementedError):
...
def test_spread_option_atm_fwd_rate():
    ...
    assert_series_equal(
        pd.Series([1, 2, 3], index=_index * 3, name="spreadOptionAtmFwdRate"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    with pytest.raises(NotImplementedError):
...
def test_zc_inflation_swap_rate():
    ...
    assert_series_equal(
        pd.Series([1, 2, 3], index=_index * 3, name="inflationSwapRate"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    with pytest.raises(NotImplementedError):
...
def test_var_swap_tenors():
    ...
    get_mock.return_value = {
        "data": [
            {
                "dataField": "varSwap",
                "filteredFields": [{"field": "tenor", "values": ["abc", "xyc"]}],
            }
        ]
    }
...
def test_forward_var_term():
    ...
    # FX
    expected_fx = pd.Series(
        [np.nan, 5.29150, 6.55744, 7.24569],
        name="forwardVarTerm",
        index=pd.DatetimeIndex(
            ["2020-05-01", "2020-06-02", "2020-07-02", "2020-09-02"],
            name="expirationDate",
        ),
    )
...
def test_var_swap_fwd():
    ...
    # regular
    idx = pd.date_range(start="2019-01-01", periods=4, freq="D")
    d1 = {"varSwap": [1, 2, 3, 4], "tenor": ["1y"] * 4}
    d2 = {"varSwap": [1.5, 2.5, 3.5, 4.5], "tenor": ["13m"] * 4}
...
def test_forward_vol():
    ...
    with DataContext("2020-01-01", "2020-09-01"):
        actual = tm.forward_vol(
            Index("MA123", AssetClass.Equity, "123"),
            "1m",
            "2m",
    ...
    market_mock.return_value = MarketDataResponseFrame(
        data={"impliedVolatility": [2.1, 3.1, 5.1], "tenor": ["1m", "2m", "4m"]},
...
def _vol_term_typical(reference, value):
    assert DataContext.current_is_set
    data = {"tenor": ["1w", "2w", "1y", "2y"], "impliedVolatility": [1, 2, 3, 4]}
    out = MarketDataResponseFrame(data=data, index=pd.DatetimeIndex(["2018-01-01"] * 4))
    out.dataset_ids = _test_datasets
...
def test_bucketize_price():
    ...
    with DataContext(datetime.date(2019, 5, 1), datetime.date(2019, 5, 1)):
        ...
        assert_series_equal(
            pd.Series(target["MISO 7x24"], index=[datetime.date(2019, 5, 1)], name="price"),
            pd.Series(actual),
        )

        actual = tm.bucketize_price(mock_miso, "LMP", bucket="offpeak")
        assert_series_equal(
            pd.Series(target["MISO offpeak"], index=[datetime.date(2019, 5, 1)], name="price"),
            pd.Series(actual),
        )
        ...
        assert_series_equal(
            pd.Series(target["CAISO 7x24"], index=[datetime.date(2019, 5, 1)], name="price"),
            pd.Series(actual),
        )

        actual = tm.bucketize_price(mock_caiso, "LMP", bucket="peak")
        assert_series_equal(
            pd.Series(target["CAISO peak"], index=[datetime.date(2019, 5, 1)], name="price"),
            pd.Series(actual),
        )
        ...
        assert_series_equal(
            pd.Series(target["7x24"], index=[datetime.date(2019, 5, 1)], name="price"),
            pd.Series(actual),
        )

        actual = tm.bucketize_price(mock_pjm, "LMP", bucket="offpeak")
        assert_series_equal(
            pd.Series(target["offpeak"], index=[datetime.date(2019, 5, 1)], name="price"),
            pd.Series(actual),
        )

        actual = tm.bucketize_price(mock_pjm, "LMP", bucket="peak")
        assert_series_equal(
            pd.Series(target["peak"], index=[datetime.date(2019, 5, 1)], name="price"),
            pd.Series(actual),
        )

        actual = tm.bucketize_price(mock_pjm, "LMP", bucket="7x8")
        assert_series_equal(
            pd.Series(target["7x8"], index=[datetime.date(2019, 5, 1)], name="price"),
            pd.Series(actual),
        )

        actual = tm.bucketize_price(mock_pjm, "LMP", bucket="2x16h")
        assert_series_equal(pd.Series(target["2x16h"], index=[], name="price"), pd.Series(actual))

        actual = tm.bucketize_price(mock_pjm, "LMP", granularity="m", bucket="7X24")
        assert_series_equal(pd.Series(target["monthly"], index=[], name="price"), pd.Series(actual))
...
def test_forward_price():
    ...
    with DataContext(datetime.date(2019, 1, 2), datetime.date(2019, 1, 2)):
        ...
        actual = tm.forward_price(mock_spp, price_method="LMP", contract_range="2Q20", bucket="7x24")
        assert_series_equal(
            pd.Series(target["7x24"], index=[datetime.date(2019, 1, 2)], name="price"),
            pd.Series(actual),
        )

        actual = tm.forward_price(mock_spp, price_method="LMP", contract_range="J20", bucket="7x24")
        assert_series_equal(
            pd.Series(target["J20 7x24"], index=[datetime.date(2019, 1, 2)], name="price"),
            pd.Series(actual),
        )

        actual = tm.forward_price(mock_spp, price_method="LMP", contract_range="2Q20", bucket="PEAK")
        assert_series_equal(
            pd.Series(target["peak"], index=[datetime.date(2019, 1, 2)], name="price"),
            pd.Series(actual),
        )

        actual = tm.forward_price(mock_spp, price_method="LMP", contract_range="J20-K20", bucket="7x24")
        assert_series_equal(
            pd.Series(target["J20-K20 7x24"], index=[datetime.date(2019, 1, 2)], name="price"),
            pd.Series(actual),
        )

        actual = tm.forward_price(mock_spp, price_method="LMP", contract_range="J20-K20", bucket="offpeak")
        assert_series_equal(
            pd.Series(
                target["J20-K20 offpeak"],
        ...
        actual = tm.forward_price(mock_spp, price_method="LMP", contract_range="J20-K20", bucket="7x8")
        assert_series_equal(
            pd.Series(target["J20-K20 7x8"], index=[datetime.date(2019, 1, 2)], name="price"),
            pd.Series(actual),
        )
        ...
        with pytest.raises(ValueError):
            tm.forward_price(mock_spp, price_method="LMP", contract_range="5Q20", bucket="PEAK")

        with pytest.raises(ValueError):
            tm.forward_price(mock_spp, price_method="LMP", contract_range="Invalid", bucket="PEAK")

        with pytest.raises(ValueError):
            tm.forward_price(mock_spp, price_method="LMP", contract_range="3H20", bucket="7x24")

        with pytest.raises(ValueError):
            tm.forward_price(mock_spp, price_method="LMP", contract_range="F20-I20", bucket="7x24")
...
def test_implied_vol_commod():
    ...
    with DataContext(datetime.date(2019, 1, 2), datetime.date(2019, 1, 2)):
        actual = tm.implied_volatility(mock, tenor="F21-H21")
        assert_series_equal(
            pd.Series(target["F21-H21"], index=[datetime.date(2019, 1, 2)], name="price"),
            pd.Series(actual),
        )
    replace.restore()
...
def test_fair_price():
    ...
    with DataContext(datetime.date(2019, 1, 2), datetime.date(2019, 1, 2)):
        actual = tm.fair_price(mock, tenor="F21")
        assert_series_equal(
            pd.Series(target["F21"], index=[datetime.date(2019, 1, 2)], name="price"),
            pd.Series(actual),
        )
    with pytest.raises(ValueError):
        tm.fair_price(mock, tenor=None)
    replace("gs_quant.timeseries.measures.GsDataApi.get_market_data", mock_fair_price_swap)
    with DataContext(datetime.date(2019, 1, 2), datetime.date(2019, 1, 2)):
        actual = tm.fair_price(mock2)

        assert_series_equal(
            pd.Series([2.880], index=[pd.Timestamp("2019-01-02")], name="fairPrice"),
            pd.Series(actual),
        )
...
def test_central_bank_swap_rate(mocker):
    ...
    with DataContext(dt.date(2019, 12, 6), dt.date(2019, 12, 6)):
        ...
        actual_abs = tm.central_bank_swap_rate(
            mock_eur, tm.MeetingType.MEETING_FORWARD, "absolute", dt.date(2019, 12, 6)
        )
        ...
        actual_rel = tm.central_bank_swap_rate(
            mock_eur, tm.MeetingType.MEETING_FORWARD, "relative", dt.date(2019, 12, 6)
        )
...
def test_policy_rate_expectation(mocker):
    ...
    with DataContext(dt.date(2019, 12, 6), dt.date(2019, 12, 6)):
        ...
        actual_date = tm.policy_rate_expectation(
            mock_eur, tm.MeetingType.MEETING_FORWARD, "absolute", dt.date(2020, 1, 23)
        )
        ...
        assert_allclose(
            [target["meeting_number_relative"]],
        ...
        mocker.patch.object(
            Dataset,
            "get_data",
            side_effect=[mock_meeting_expectation(), mock_empty_market_data_response()],
        )
        with pytest.raises(MqError):
...
def test_esg_aggregate():
    ...
    assert_series_equal(
        pd.Series([2, 4, 6], index=_index * 3, name="esMomentumScore"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    ...
    assert_series_equal(
        pd.Series([81.2, 75.4, 65.7], index=_index * 3, name="esNumericPercentile"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    ...
    assert_series_equal(
        pd.Series([81.2, 75.4, 65.7], index=_index * 3, name="esPolicyPercentile"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    ...
    assert_series_equal(
        pd.Series([81.2, 75.4, 65.7], index=_index * 3, name="esPercentile"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    ...
    assert_series_equal(
        pd.Series([81.2, 75.4, 65.7], index=_index * 3, name="gPercentile"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    ...
    assert_series_equal(
        pd.Series([81.2, 75.4, 65.7], index=_index * 3, name="esMomentumPercentile"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
    ...
    assert_series_equal(
        pd.Series([81.2, 75.4, 65.7], index=_index * 3, name="gRegionalPercentile"),
        pd.Series(actual),
    )
    assert actual.dataset_ids == _test_datasets
...
def test_gir_gsdeer_gsfeer(mocker):
    ...
    actual = tm.gir_gsdeer_gsfeer(mock_usdeur, tm.EquilibriumExchangeRateMetric.GSDEER)
    assert_series_equal(pd.Series([1, 1.2, 1.1], index=index, name="gsdeer"), pd.Series(actual))

    actual = tm.gir_gsdeer_gsfeer(mock_usdeur, tm.EquilibriumExchangeRateMetric.GSFEER)
    assert_series_equal(pd.Series([2, 1.8, 1.9], index=index, name="gsfeer"), pd.Series(actual))

    actual = tm.gir_gsdeer_gsfeer(mock_eurusd, tm.EquilibriumExchangeRateMetric.GSDEER)
    assert_series_equal(
        pd.Series([1 / 1, 1 / 1.2, 1 / 1.1], index=index, name="gsdeer"),
        pd.Series(actual),
    )

    actual = tm.gir_gsdeer_gsfeer(mock_eurusd, tm.EquilibriumExchangeRateMetric.GSFEER)
    assert_series_equal(
        pd.Series([1 / 2, 1 / 1.8, 1 / 1.9], index=index, name="gsfeer"),
        pd.Series(actual),
    )
    with pytest.raises(NotImplementedError):
        tm.gir_gsdeer_gsfeer(mock_usdeur, tm.EquilibriumExchangeRateMetric.GSDEER, real_time=True)
    replace.restore()
...
def test_gir_commodities_forecast():
    ...
    actual = tm.gir_commodities_forecast(mock_spgcsb, "3m", tm._CommoditiesForecastType.SPOT_RETURN)
    assert_series_equal(
        pd.Series(
            [1700, 1400, 1500, 1600],
            index=pd.to_datetime(
            ...
            name="girCommoditiesForecast",
        ),
        pd.Series(actual),
    )

    with pytest.raises(NotImplementedError):
        tm.gir_commodities_forecast(mock_spgcsb, "3m", tm._CommoditiesForecastType.SPOT_RETURN, real_time=True)
    replace.restore()


def mock_forward_curve_peak(_cls, _q):
    d = {"forwardPrice": [40.05], "quantityBucket": ["PEAK"], "contract": ["U20"]}
...
def mock_forward_curve_peak_holiday(_cls, _q):
    d = {"forwardPrice": [26.567302], "quantityBucket": ["PEAK"], "contract": ["U20"]}
