...
import pandas as pd
import pytest
from numpy import datetime64, float64, int64, object
...
test_data = [{'date': dt.date(2019,
              ...
              'updateTime': dt.datetime.strptime('2019-01-03T00:53:00Z',
                                                 '%Y-%m-%dT%H:%M:%SZ')},
             {'date': dt.date(2019,
                              1,
                              3),
              ...
              'updateTime': dt.datetime.strptime('2019-01-04T00:14:00Z',
                                                 '%Y-%m-%dT%H:%M:%SZ')},
             {'date': dt.date(2019,
                              1,
                              4),
              ...
              'updateTime': dt.datetime.strptime('2019-01-08T00:31:00Z',
                                                 '%Y-%m-%dT%H:%M:%SZ')},
             {'date': dt.date(2019,
                              1,
                              7),
             ...
             {'date': dt.date(2019,
              ...
              'updateTime': dt.datetime.strptime('2019-01-09T00:50:00Z',
                                                 '%Y-%m-%dT%H:%M:%SZ')},
             {'date': dt.date(2019,
                              1,
                              9),
              ...
              'updateTime': dt.datetime.strptime('2019-01-10T00:44:00Z',
                                                 '%Y-%m-%dT%H:%M:%SZ')}]
...
def test_query_data(mocker):
    mocker.patch(
        "gs_quant.api.gs.data.GsDataApi.query_data",
        return_value=test_data)
    mocker.patch(
        "gs_quant.api.gs.data.GsDataApi.get_types",
        return_value=test_types)
    dataset = Dataset(Dataset.TR.TREOD)
    data = dataset.get_data(
        dt.date(
            2019, 1, 2), dt.date(
            2019, 1, 9), assetId='MA4B66MW5E27U8P32SB')
    assert data.equals(GsDataApi.construct_dataframe_with_types(
        str(Dataset.TR.TREOD), test_data))
...
def test_last_data(mocker):
    mocker.patch("gs_quant.api.gs.data.GsDataApi.last_data",
                 return_value=[test_data[-1]])
    mocker.patch(
        "gs_quant.api.gs.data.GsDataApi.get_types",
        return_value=test_types)
    dataset = Dataset(Dataset.TR.TREOD)
    data = dataset.get_data_last(
        dt.date(2019, 1, 9), assetId='MA4B66MW5E27U8P32SB')
    assert data.equals(GsDataApi.construct_dataframe_with_types(
        str(Dataset.TR.TREOD), ([test_data[-1]])))
...
def test_get_coverage(mocker):
    mocker.patch(
        ...
        return_value=test_coverage_data)
    mocker.patch(
        "gs_quant.api.gs.data.GsDataApi.get_types",
        return_value={
            'gsid': 'string'})
    ...
    gsid = GsDataApi.construct_dataframe_with_types(
        str(Dataset.TR.TREOD), results).get('gsid').get(0)
...
def test_construct_dataframe_with_types(mocker):
    ...
    df = GsDataApi.construct_dataframe_with_types(
        str(Dataset.TR.TREOD), [test_data[0]])
    ...
    # https://pbpython.com/pandas_dtypes.html python str == dtype object
    assert df['assetId'].dtype == object
