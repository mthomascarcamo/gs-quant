...
from gs_quant.risk import (CompositeScenario, DataFrameWithInfo, ErrorValue,
                           FloatWithInfo, MarketDataScenario, RiskMeasure,
                           StringWithInfo)
from gs_quant.risk.results import PricingFuture
from gs_quant.session import GsSession
from gs_quant.target.common import PricingDateAndMarketDataAsOf
from gs_quant.target.risk import (RiskPosition, RiskRequest,
...
class PricingCache(metaclass=ABCMeta):
    ...
    def get(
            cls,
            risk_key: RiskKey,
            instrument: InstrumentBase) -> Optional[CacheResult]:
...
class PricingContext(ContextBaseWithDefault):
    ...
    def __init__(self,
        ...
        **Examples**
        ...
        if market and market_data_location and market.location is not market_data_location:
            raise ValueError(
                'market.location and market_data_location cannot be different')
        ...
        self.__pricing_date = pricing_date or (
            self.prior_context.pricing_date if self.prior_context else business_day_offset(
                dt.date.today(), 0, roll='preceding'))
        ...
        self.__market_data_location = get_enum_value(
            PricingLocation, market_data_location)
        self.__market = market or CloseMarket(
            date=close_market_date(
                self.__market_data_location,
                self.__pricing_date) if pricing_date else None,
    ...
    def __calc(self):
        def run_requests(requests_: list, provider_, create_event_loop: bool):
            ...
            try:
                with session:
                    results = provider_.run(
                        requests_, 1000, progress_bar, timeout=self.__timeout)
        ...
        for (key, instrument) in self.__pending.keys():
            requests_by_provider.setdefault(
                key.provider,
                {}) .setdefault(
                (key.params,
                 key.scenario,
        ...
        if requests_by_provider:
            for provider, by_params_scenario_date_market in requests_by_provider.items():
                ...
                for (params, scenario, date,
                     market), positions_by_measures in by_params_scenario_date_market.items():
                    for instrument, risk_measures in positions_by_measures.items():
                        grouped_requests.setdefault(
                            (params, scenario, date, market, tuple(
                                sorted(risk_measures))), []).append(instrument)

                requests = []

                for (params, scenario, date, market,
                     risk_measures), instruments in grouped_requests.items():
                    for chunk in [tuple(filter(None, i)) for i in zip_longest(
                            *[iter(instruments)] * 1000)]:
                        requests.append(
                            RiskRequest(
                                tuple(
                                    RiskPosition(
                                        instrument=i,
                                        quantity=i.instrument_quantity) for i in chunk),
                                risk_measures,
            ...
            show_status = self.__show_progress and (
                len(requests_for_provider) > 1 or len(
                    next(
                        iter(
                            requests_for_provider.values()))) > 1)
    ...
    def _parameters(self) -> RiskRequestParameters:
        return RiskRequestParameters(
            csa_term=self.__csa_term, raw_results=True)
    ...
    def _scenario(self) -> Optional[MarketDataScenario]:
        ...
        return MarketDataScenario(
            scenario=scenarios[0] if len(scenarios) == 1 else CompositeScenario(
                scenarios=tuple(
                    reversed(scenarios))))
    ...
    def active_context(self):
        return next(
            (c for c in reversed(
                PricingContext.path) if c.is_entered),
            self)
    ...
    def clone(self, **kwargs):
        clone_kwargs = {
            k: getattr(
                self, k, None) for k in signature(
                self.__init__).parameters.keys()}
