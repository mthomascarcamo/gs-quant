...
class AddTradeAction(Action):
    def __init__(self,
                 priceables: Union[Priceable,
                                   Iterable[Priceable]],
                 trade_duration: str = None,
                 name: str = None):
        """
    ...
    def apply_action(self,
        ...
        with PricingContext(is_batch=True):
            f = {}
            for s in state:
                active_portfolio = self._dated_priceables.get(
                    s) or self._priceables
        ...
        for s in backtest.states:
            ...
            for create_date, portfolio in f.items():
                pos += [inst for inst in portfolio.result().instruments if get_final_date(
                    inst, create_date, self.trade_duration) >= s >= create_date]
            if len(pos) > 0:
...
class HedgeAction(Action):
    def __init__(
            self,
            risk,
            priceables: Priceable,
            trade_duration: str = None,
            risks_on_final_day: bool = False,
            name: str = None):
        super().__init__(name)
        ...
        if self._priceable.name is None:
            ...
            self._priceable.name = '{}_{}'.format(
                self._name, self._priceable.name)
    ...
    def apply_action(self,
        ...
        for create_date, portfolio in f.result().items():
            active_dates = [
                s for s in backtest.states if get_final_date(
                    portfolio.instruments[0],
                    create_date,
                    self.trade_duration) >= s >= create_date]
            backtest.scaling_portfolios[create_date].append(ScalingPortfolio(
                trade=portfolio.instruments[0], dates=active_dates, risk=self.risk))
