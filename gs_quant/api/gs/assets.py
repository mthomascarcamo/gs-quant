...
from gs_quant.target.assets import (AssetClass, AssetParameters,
                                    AssetToInstrumentResponse, AssetType,
                                    Currency, EntityQuery, Position,
                                    PositionSet, TemporalXRef)
...
# prevent collection from growing without bound
invocation_locks = cachetools.LRUCache(1024)


def _cached(fn):
    _fn_cache_lock = threading.Lock()
    # short-term cache to avoid retrieving the same data several times in
    # succession
    cache = cachetools.TTLCache(1024, 30) if os.environ.get(
        'GSQ_SEC_MASTER_CACHE') else None

    @wraps(fn)
    def wrapper(*args, **kwargs):
        if cache is not None:
            args = [
                tuple(x) if isinstance(
                    x, list) else x for x in args]  # tuples are hashable
            k = cachetools.keys.hashkey(GsSession.current, *args, **kwargs)
            with metalock:
                invocation_lock = invocation_locks.setdefault(
                    f'{fn.__name__}:{k}', threading.Lock())
...
class GsAssetApi:
    ...
    def __create_query(
        ...
        valid = keys.intersection(
            i for i in dir(FieldFilterMap) if isinstance(
                getattr(
                    FieldFilterMap, i), property))
        invalid = keys.difference(valid)

        if invalid:
            bad_args = ['{}={}'.format(k, kwargs[k]) for k in invalid]
            raise KeyError(
                'Invalid asset query argument(s): {}'.format(
                    ', '.join(bad_args)))
    ...
    def get_many_assets(
        ...
        response = GsSession.current._post(
            '/assets/query', payload=query, cls=return_type)
    ...
    def get_asset_xrefs(
        ...
        response = GsSession.current._get(
            '/assets/{id}/xrefs'.format(id=asset_id))
        return tuple(GsTemporalXRef.from_dict(x)
                     for x in response.get('xrefs', ()))
    ...
    def get_asset(
        ...
        return GsSession.current._get(
            '/assets/{id}'.format(id=asset_id), cls=GsAsset)
    ...
    def get_asset_positions_for_date(
        ...
        url = '/assets/{id}/positions/{date}'.format(
            id=asset_id, date=position_date_str)
    ...
    def get_latest_positions(
            cls, asset_id: str, position_type: str = 'close') -> Union[PositionSet, dict]:
        url = '/assets/{id}/positions/last?type={ptype}'.format(
            id=asset_id, ptype=position_type)
        results = GsSession.current._get(url)['results']
    ...
    def get_instruments_for_asset_ids(
        ...
        instrument_infos = GsSession.current._post(
            '/assets/instruments', asset_ids, cls=AssetToInstrumentResponse)
        instrument_lookup = {
            i.assetId: i.instrument for i in instrument_infos if i}
        ret: Tuple[Optional[Union[Instrument, Security]]] = tuple(
            instrument_lookup.get(a) for a in asset_ids)
    ...
    def get_instruments_for_positions(
        ...
        instrument_infos = GsSession.current._post(
            '/assets/instruments',
            asset_ids,
            cls=AssetToInstrumentResponse) if asset_ids else {}

        instrument_lookup = {
            i.assetId: (
                i.instrument,
                i.sizeField) for i in instrument_infos if i}
        ret = ()
        ...
        for position in positions:
            ...
            if position.instrument:
                ...
                if instrument_info:
                    ...
                    if instrument is not None and size_field is not None and getattr(
                            instrument, size_field, None) is None:
    ...
    def map_identifiers(
        ...
        query = cls.__create_query(
            (input_type, output_type), as_of, limit, **the_args)
        ...
        for entry in results:
            ...
            if multimap:
                ...
                if key in out:
                    _logger.warning(
                        '%s: more than one mapping for %s',
                        GsAssetApi.map_identifiers.__name__,
                        key)
                out[key] = value
        return out
